...
func InstallBuiltinConfigs(path string) error {
	...
	for name, config := range builtinConfigs {
		...
		if err != nil {
			errs = errors.Join(errs, fmt.Errorf("write %s: %v", path, err))
			continue
		}
	}
	return errs
...
var AutomatedConfig = ProcessingConfig{
	Choices: []Choice{
		...
		{
			Comment:   "Virus scanning: No",
			AppliesTo: "856d2d65-cd25-49fa-8da9-cabb78292894",
			GoToChain: "63767e4b-9ce8-4fe2-8724-65cc1f763de0",
		},
		{
			Comment:   "Virus scanning: No",
			AppliesTo: "1dad74a2-95df-4825-bbba-dca8b91d2371",
			GoToChain: "697c0883-798d-4af7-b8b6-101c7f709cd5",
		},
		{
			Comment:   "Virus scanning: No",
			AppliesTo: "7e81f94e-6441-4430-a12d-76df09181b66",
			GoToChain: "77355172-b437-4324-9dcc-e2607ad27cb1",
		},
		{
			Comment:   "Virus scanning: No",
			AppliesTo: "390d6507-5029-4dae-bcd4-ce7178c9b560",
			GoToChain: "63be6081-bee8-4cf5-a453-91893e31940f",
		},
		{
			Comment:   "Virus scanning: No",
			AppliesTo: "97a5ddc0-d4e0-43ac-a571-9722405a0a9b",
			GoToChain: "7f5244fe-590b-4e38-beaf-0cf1ccb9e71b",
		},
	},
}
