...
def test_report_is_always_stored(mocker, args, test_case):
    mocker.patch(
        "worker.clientScripts.email_fail_report.get_emails_from_dashboard_users",
        return_value=test_case["users"],
    )
    mocker.patch(
        "worker.clientScripts.email_fail_report.get_content_for",
        return_value="my report",
    )
    send_email_mock = mocker.patch(
        "worker.clientScripts.email_fail_report.send_email",
        side_effect=test_case["send_email_result"],
    )
    store_report_mock = mocker.patch(
        "worker.clientScripts.email_fail_report.store_report"
    )
