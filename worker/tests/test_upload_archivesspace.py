...
def test_get_files_from_dip_with_empty_dip_location(tmpdir, mocker):
    logger = mocker.patch("worker.clientScripts.upload_archivesspace.logger")
...
def test_upload_to_archivesspace_adds_trailing_slash_to_uri(db, mocker, uri):
    ...
    mocker.patch("worker.clientScripts.upload_archivesspace.mets_file")
    mocker.patch(
        "worker.clientScripts.upload_archivesspace.get_pairs",
        return_value={file_uuid: "myresource"},
    )
...
def test_upload_to_archivesspace_gets_mets_if_needed(mocker, params):
    mocker.patch("worker.clientScripts.upload_archivesspace.get_pairs")
    logger = mocker.patch("worker.clientScripts.upload_archivesspace.logger")
    mets_file_mock = mocker.patch("worker.clientScripts.upload_archivesspace.mets_file")
...
def test_call(db, mocker):
    ...
    mocker.patch(
        "worker.clientScripts.upload_archivesspace.get_parser", return_value=parser_mock
    )
    client_mock = mocker.Mock()
    client_factory_mock = mocker.patch(
        "worker.clientScripts.upload_archivesspace.ArchivesSpaceClient",
...
def test_call_when_files_from_dip_cant_be_retrieved(db, mocker, params):
    mocker.patch("worker.clientScripts.upload_archivesspace.get_parser")
    mocker.patch("worker.clientScripts.upload_archivesspace.ArchivesSpaceClient")
    mocker.patch(
        "worker.clientScripts.upload_archivesspace.get_files_from_dip",
        side_effect=params["exception"],
    )
    upload_to_archivesspace = mocker.patch(
        "worker.clientScripts.upload_archivesspace.upload_to_archivesspace"
    )
...
def test_call_when_not_all_files_can_be_paired(db, mocker):
    mocker.patch("worker.clientScripts.upload_archivesspace.get_parser")
    mocker.patch("worker.clientScripts.upload_archivesspace.ArchivesSpaceClient")
    mocker.patch("worker.clientScripts.upload_archivesspace.get_files_from_dip")
    mocker.patch(
        "worker.clientScripts.upload_archivesspace.upload_to_archivesspace",
        return_value=False,
    )
